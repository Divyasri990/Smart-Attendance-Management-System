...
dataset_path = "C:/Users/sridi/PycharmProjects/MLDL_project/dataset"
...
def recognize_faces(frame):
    ...
    for face_encoding in face_encodings:
        matches = face_recognition.compare_faces([item for sublist in dataset.values() for item in sublist],
                                                 face_encoding)
        name = "Unknown"
        if True in matches:
...
log_file_path = 'C:/Users/sridi/PycharmProjects/MLDL_project/log.csv'
column_names = ['Year', 'Month', 'Day', 'Time', 'Name', 'Attendance']
...
def mark_attendance(name):
    ...
    with open(log_file_path, mode='a', newline='') as logFile:
        pos = logFile.tell()
        csv_writer = csv.writer(logFile)  # Define the csv_writer here
        if pos == 0:
            csv_writer.writerow(column_names)
        info = [year, month, day, time1, name, "Present"]
